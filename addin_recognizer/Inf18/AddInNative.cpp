...
static const wchar_t g_ComponentNameAddIn[] = L"SpeechRecognizer";
...
bool AddInNative::Init(void* pConnection)
{
	...
	{		
		...
		static const wchar_t g_EventName[] = L"initialization";
		static WcharWrapper s_EventSource(g_EventSource);
		static WcharWrapper s_EventName(g_EventName);
		m_iConnect->ExternalEvent(s_EventSource, s_EventName, nullptr);
		return true;
	}
...
bool AddInNative::CallAsProc(const long lMethodNum, tVariant* paParams, const long lSizeArray)
{
	switch (lMethodNum)
	{
		...
		case eMethPlaySoundAlert: {
			javaMainApp.playSoundAlert();
			return true;
		}
		case eMethRequestServiceStatus: {
			javaMainApp.requestServiceStatus();
			return true;
		}
		default:
			return false;
